...
public class ManagerController implements Initializable{
	...
	private IFormInfoService service = new FormInfoService();
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		search.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				...
				if(name_text.equals("") && phone_text.equals("")) {
					...
					List<Form_info> list_form_filter = service.searchForm(form);
					List<FormTable> list_after = list_form_filter.stream().map(f -> new FormTable(f)).collect(Collectors.toCollection(ArrayList::new));
		...
		pay.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				if(store.getForm() != null) Redirect();
			}
		});
	}
